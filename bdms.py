...
def rho_z(z, zvir, dmax = 200., dz = 20):
	rho_vir = dmax * rhom(1/(1+zvir))
	out = delta(z, zvir, dmax) * rhom(1/(1+z))
	return rho_vir * ((z-zvir)<=dz) * np.logical_or((out>=rho_vir), z<=zvir) + out * np.logical_or(((z-zvir)>dz), (z>zvir)*(out<=rho_vir))

def Dlnrho(t1, t2, zvir, dmax = 200.):
	z1 = z_t(t1)
	z2 = z_t(t2)
	dt = t2 - t1
	drho = rho_z(z2, zvir, dmax)-rho_z(z1, zvir, dmax)
	rho = rho_z(z1, zvir, dmax)
	#if dt<=0 or rho<=0:
	#	return 0
	#else:
	return drho/(rho*dt)
...
def bdmscool(Tdm, Tb, v, z, rhob, rhodm, Mdm, sigma, gamma, xh, Om, Ob, h):
	a = 1/(1+z)
	rho = rhob + rhodm
